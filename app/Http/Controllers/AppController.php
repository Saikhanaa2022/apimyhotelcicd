...
class AppController extends Controller
{
    ...
    public function dracula($key) {
        ...
        if ($key === 'fireNotify') {
            // Find user by email
            $gid = config('app.env') == 'production' ? 20097 : 395;
            $reservation = \App\Models\Reservation::where('group_id',$gid)->first();
            ...
            $user = \App\Models\User::whereEmail('dev@ihotel.mn')->first();

            // Notify to user
            if (!is_null($user)) {
                $user->notify(new \App\Notifications\XroomNotification($emailData, $reservation));
            }
